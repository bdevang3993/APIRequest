...
class APIRequestURL {
    ...
    func postRequest<T:Decodable>(serviceName:String,httpMethod:String,andParams params: [String:Any],success successBlock: @escaping ((T) -> Void), andFailureBlock failedBlock:@escaping ((String) -> Void)){
        if isConnectedToInterNet() {
            guard let url = URL(string: serviceName) else {
                failedBlock(ErrorMessages.urlSide.value())
                return
